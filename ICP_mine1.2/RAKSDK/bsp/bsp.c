...
PUTCHAR_PROTOTYPE
{
    ...
    /* Place your implementation of fputc here */
    /* e.g. write a character to the EVAL_COM1 and Loop until the end of transmission */
    //HAL_UART_Transmit(&huart1, (uint8_t *)&ch, 1, 0xFFFF); //中断中会造成阻塞
	  while((USART6->SR & 0x40)==0){};//循环发送，直到发送完成为止
...
//  @ input:
//  @ output:
//  @ note: 注意不能直接用MX_GPIO_Init();会把其他GPIO初始化了
void WIFI_GPIO_Init(void){
  ...
  GPIO_InitStruct.Mode = GPIO_MODE_IT_FALLING;//注意中断是下降沿触发
